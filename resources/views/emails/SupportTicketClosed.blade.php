...
#contBtn { position: relative; top: 1.5em; text-decoration: none; background: #2cc3a9; color: #fff; margin: auto; padding: .8em 3em; -webkit-box-shadow: 0px 15px 30px rgba(50, 50, 50, 0.21); -moz-box-shadow: 0px 15px 30px rgba(50, 50, 50, 0.21); box-shadow: 0px 15px 30px rgba(50, 50, 50, 0.21); border-radius: 25px; -webkit-transition: all .4s ease; -moz-transition: all .4s ease; -o-transition: all .4s ease; transition: all .4s ease; } #contBtn:hover { -webkit-box-shadow: 0px 15px 30px rgba(60, 60, 60, 0.40); -moz-box-shadow: 0px 15px 30px rgba(60, 60, 60, 0.40); box-shadow: 0px 15px 30px rgba(60, 60, 60, 0.40); -webkit-transition: all .4s ease; -moz-transition: all .4s ease; -o-transition: all .4s ease; transition: all .4s ease; }
    ...
    <div id='lower-side'>   
        ...
        <p id='message' style="text-align: left">We are writing to inform you that your support ticket with the reference number{{$ticketNum}} has been successfully resolved and is now closed.</p>
        <p id='message' style="text-align: left">Ticket Details</p>
        <p id='message' style="text-align: left">Ticket Title:{{$ticketTitle}}</p>
        <p id='message' style="text-align: left">Ticket Number:{{$ticketNum}}</p>
        <p id='message' style="text-align: left">Ticket Description:{{$ticketDescription}}</p>       
        <p id='message' style="text-align: left">You can view the Ticket Details by clicking <a href="{{$link}}">here</a></p>
        <p id='message' style="text-align: left">If you are unable to access the link, you can copy and paste it into your web browser to view the ticket comments.</p>
